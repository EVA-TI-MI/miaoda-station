...
弈境棋院（AI 围棋 / AI 中国象棋 / AI 国际象棋 / AI 五子棋）+ 腾蛇 + 墨渊阁小说阅读器。
...
## 版本
- V4.1.3：修复围棋在手机上由 19 路切换到 13 路 / 9 路时网格缩在画布左上角、无法与页面贴合的问题（切换路数后重新计算格子间距与画布布局）；墨渊阁手机端阅读设置界面点按体验优化（触控热区加大至 38–44px、滑块手柄加粗加大、新增按压反馈），页面标题去掉“- 免费小说阅读”；“霓虹贪吃蛇”更名为“腾蛇”（页面文件名 snake.html 不变）；PWA 缓存升级 v23
- V4.1.2：棋类移动端适配——修复围棋/中国象棋/国际象棋/五子棋棋盘在手机（微信内置浏览器等）上固定像素宽度溢出、右侧与底部被裁切的问题；棋盘改为容器驱动的响应式尺寸（CSS 强制适配 + ResizeObserver 兜底内核首次视口误报），落子/悬停坐标按实际显示尺寸换算保证触屏点按准确；桌面端布局与棋盘尺寸保持不变；PWA 缓存升级 v22
